...
export default function AddMemberModal({ onAdd, onClose }: { onAdd: (data: Member) => void, onClose: () => void }) {
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (form.firstName.length < 3) {
      setError("First name must be more then 3 characters");
      return;
    }

    if (!form.firstName || !form.familyName || !form.email || !form.phoneNumber || !form.apogee) {
    ...
    try {
      ...
      console.log(err);
      setError(
        "Something went wrong, could not add the new member. Please contact the developer."
      );
    } finally {
